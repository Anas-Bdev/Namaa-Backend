...
public class SetActualResultsCommandHandler(
    IAppDbContext context,
    IUserReadRepository userReadRepository)
    : IRequestHandler<SetActualResultsCommand, Result<InvestmentProjectDto>>
{
    public async Task<Result<InvestmentProjectDto>> Handle(
        SetActualResultsCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var result = project.SetActualResults(request.ActualRevenue, request.ActualCost);
        if (result.IsError)
